...
activity_data = pd.read_csv('data/trainningdata.csv')

missing_value_describe(activity_data)

# Normalize column names (strip whitespace, convert to lowercase)
activity_data.columns = activity_data.columns.str.strip().str.lower()
...
# class distribution
#print(activity_data.groupby('Species').size())
...
X = activity_data.drop(['activity'], axis=1)
...
models.append(('KNN', KNeighborsClassifier(n_neighbors=5, weights='distance', metric='euclidean')))   #k-nearest neighbors
...
def test_model(name, model):
    ...
    # Export model
    joblib.dump(model, gzip.open('model/Activity-' + name + '.dat.gz', "wb"))
